  ...
  CREATE TABLE IF NOT EXISTS `corrida_db`.`corredores` (
    `idcorredores` INT NOT NULL AUTO_INCREMENT,
    `nome` VARCHAR(255) NOT NULL,
    `turma` VARCHAR(45) NOT NULL,
    PRIMARY KEY (`idcorredores`))
  ENGINE = InnoDB;
  ...
  CREATE TABLE IF NOT EXISTS `corrida_db`.`voltas` (
    `idvoltas` INT NOT NULL AUTO_INCREMENT,
    `tempo` DECIMAL(0,2) NOT NULL,
    `data` TIMESTAMP NOT NULL,
    `corredores_idcorredores` INT NOT NULL,
    PRIMARY KEY (`idvoltas`, `corredores_idcorredores`),
    INDEX `fk_voltas_corredores_idx` (`corredores_idcorredores` ASC) VISIBLE,
    CONSTRAINT `fk_voltas_corredores`
      FOREIGN KEY (`corredores_idcorredores`)
      REFERENCES `corrida_db`.`corredores` (`idcorredores`)
      ON DELETE NO ACTION
      ON UPDATE NO ACTION)
  ENGINE = InnoDB;
